...
let typeToString = (t: typ): string => {
  ...
  let rec typeToStringRec = (curTypeVarName, typeVarNames, x) =>
    switch x {
    | TUnit => "unit"
    | TVar({contents: Bound(t_)}) => typeToStringRec(curTypeVarName, typeVarNames, t_)
    ...
    | Fn(a, b) =>
      let aStr = typeToStringRec(curTypeVarName, typeVarNames, a)
      let bStr = typeToStringRec(curTypeVarName, typeVarNames, b)
    ...
    | PolyType(tvs, t) =>
      let curriedFn = t =>
        typeToStringRec(curTypeVarName, typeVarNames, TVar(ref(Unbound(t, currentLevel.contents))))
      let tvsStr = List.reduce(tvs, "", (s, tv) => s ++ (" '" ++ curriedFn(tv)))
      "forall" ++ (tvsStr ++ (" . " ++ typeToStringRec(curTypeVarName, typeVarNames, t)))
    }

  typeToStringRec(ref("a"), HashMap.Int.make(~hintSize=1), t)
}

let printType = (t: typ): unit => Js.log(typeToString(t))
...
let test = (name: string, ast: expr, expected: string) => {
  try {
    let typ = infer(Map.String.empty, ast)
    let correct = expected == typeToString(typ)
    Js.log(`  ${correct ? "" : name} ${correct ? "" : ":"} ${correct ? "" : typeToString(typ)}`)
    Js.log(`${correct ? " " : "x"} ${name} : ${typeToString(typ)}`)
